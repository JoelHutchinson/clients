...
export class Fido2CreateComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      ...
      await this.showErrorDialog(DIALOG_MESSAGES.unexpectedError);
    }
  }
  ...
  async confirmPasskey(): Promise<void> {
    try {
      ...
      this.logService.error("Failed to confirm passkey", error);
      await this.showErrorDialog(DIALOG_MESSAGES.unableToSave);
    }
  }
